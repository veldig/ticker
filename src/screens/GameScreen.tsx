...
export function GameScreen() {
  ...
  return (
    <>
      ...
      <main className="flex flex-col h-[calc(100dvh-56px)] overflow-hidden">

        {/* ── Scrollable content ── */}
        <div className="flex-1 overflow-y-auto">
          <div className="max-w-2xl mx-auto w-full px-4 py-3 sm:py-5 space-y-3 sm:space-y-4 pb-4">
            ...
            <section aria-label="Today's puzzle">
              <div className="bg-[#161a20] border border-white/8 rounded-2xl p-4">
                ...
                <div className="grid grid-cols-4 gap-1.5 sm:gap-2" role="presentation">
                  {(['Sector', 'Mkt Cap', 'P/E', '5Y Ret'] as const).map(label => (
                    <div
                      key={label}
                      className="flex flex-col items-center justify-center bg-white/4 border border-white/6 rounded-xl p-2 sm:p-3 min-h-[52px] sm:min-h-[62px] gap-1"
                      aria-hidden="true"
                    >
            ...
            {guesses.length > 0 ? (
              <section aria-label="Guess history">
                ...
                <ul ref={listRef} role="list" className="space-y-3 sm:space-y-4 list-none p-0 m-0">
              ...
              >
                <div className="grid grid-cols-4 gap-1.5 w-full max-w-xs opacity-20">
                  {Array.from({ length: 8 }).map((_, i) => (
                    <div key={i} className="min-h-[56px] rounded-xl border border-white/10 bg-white/3" />
                  ))}
                </div>
            ...
            {guesses.length > 0 && !isFinished && (
              <div aria-hidden="true" className="space-y-3 sm:space-y-4">
                {Array.from({ length: attemptsLeft }).map((_, i) => (
                  <div key={i} className="grid grid-cols-4 gap-1.5 sm:gap-2 pl-0 sm:pl-5">
                    {Array.from({ length: 4 }).map((_, j) => (
                      <div key={j} className="min-h-[60px] sm:min-h-[68px] rounded-xl border border-white/4 bg-white/[0.015]" />
                    ))}
                  </div>
                ))}
              </div>
        ...
        {!isFinished && (
          <div
            className="shrink-0 max-w-2xl mx-auto w-full px-4 pt-2 bg-[#0d0f12]"
